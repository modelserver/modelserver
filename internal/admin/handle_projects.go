...
type quotaWindowHistory struct {
	Window     string              `json:"window"`
	WindowType string              `json:"window_type"`
	Series     []quotaHistoryPoint `json:"series"`
}

// serveQuotaHistory returns time-series quota percentage data.
// Only percentages are returned; absolute credits are never exposed.
func serveQuotaHistory(st *store.Store, w http.ResponseWriter, projectID, userID string) {
	member, err := st.GetProjectMember(projectID, userID)
	if err != nil || member == nil {
		writeError(w, http.StatusNotFound, "not_found", "member not found")
		return
	...
	type response struct {
		UserID  string               `json:"user_id"`
		Windows []quotaWindowHistory `json:"windows"`
	}
	resp := response{UserID: userID, Windows: []quotaWindowHistory{}}
	...
	for _, rule := range policy.CreditRules {
		...
		wh := quotaWindowHistory{
			Window:     rule.Window,
			WindowType: rule.WindowType,
			Series:     []quotaHistoryPoint{},
		}

		// Pick granularity by window suffix: "h" → hourly, everything else (d/w/M) → daily.
		useHourly := strings.HasSuffix(rule.Window, "h")

		var buckets []store.BucketedCreditsUsage
		if useHourly {
			buckets, err = st.GetHourlyCreditsConsumedByUser(projectID, userID, windowStart, now)
		} else {
			buckets, err = st.GetDailyCreditsConsumedByUser(projectID, userID, windowStart, now)
		}
		if err != nil {
			buckets = nil
		}

		// Compute cumulative percentage series.
		var cumulative float64
		for _, b := range buckets {
			cumulative += b.Credits
			pct := (cumulative / float64(userLimit)) * 100
			if pct > 100 {
				pct = 100
			}
			...
			var ts string
			if useHourly {
				ts = b.Bucket.Format(time.RFC3339)
			} else {
				ts = b.Bucket.Format("2006-01-02")
			}
			wh.Series = append(wh.Series, quotaHistoryPoint{
				Timestamp:  ts,
				Percentage: pct,
			})
		}

		resp.Windows = append(resp.Windows, wh)
	}

	writeData(w, http.StatusOK, resp)
}

func handleQuotaHistory(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		projectID := chi.URLParam(r, "projectID")
		userID := chi.URLParam(r, "userID")
		caller := UserFromContext(r.Context())
		...
		serveQuotaHistory(st, w, projectID, userID)
	}
}

func handleMyQuotaHistory(st *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		projectID := chi.URLParam(r, "projectID")
		caller := UserFromContext(r.Context())
		serveQuotaHistory(st, w, projectID, caller.ID)
	}
}
